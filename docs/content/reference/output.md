...
describes what users get. Pick a format with `-o`, or let stringdb choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
stringdb <command> -o table   # aligned columns for reading
stringdb <command> -o jsonl   # one JSON object per line, for piping
stringdb <command> -o json    # a single JSON array
stringdb <command> -o csv     # spreadsheet friendly
stringdb <command> -o tsv     # tab-separated
stringdb <command> -o url     # just the URL column
stringdb <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
stringdb <command> --fields id,title,url
```
...
```bash
stringdb <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
stringdb <command>            # a table, because this is a terminal
stringdb <command> | wc -l    # JSONL, because this is a pipe
```
